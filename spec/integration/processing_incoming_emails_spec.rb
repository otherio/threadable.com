...
describe "processing incoming emails" do
  ...
  def validate! result
    ...
    if result == :bounced || result == :held || result == :dropped
      ...
      if result == :held
        if expect_message_held_notices_sent
          # held mail gets sent an auto-response
          expect(sent_emails.size).to eq 2
          held_notice = sent_emails.with_subject("[message held] #{subject}").first
          ...
          held_owner_notice = sent_emails.with_subject("[message held, action required] #{subject}").first

          expect( held_owner_notice.smtp_envelope_from ).to eq "no-reply-auto@#{threadable.email_host}"
          expect( held_owner_notice.smtp_envelope_to   ).to eq ['alice@ucsd.example.com']
          expect( held_owner_notice.to                 ).to eq ['alice@ucsd.example.com']
          expect( held_owner_notice.from               ).to eq ["support+message-held@#{threadable.email_host}"]

          expect( held_owner_notice.header['Reply-To'].to_s       ).to eq "Threadable message held <support+message-held@#{threadable.email_host}>"
          expect( held_owner_notice.header['In-Reply-To'].to_s    ).to eq incoming_email.message_id
          expect( held_owner_notice.header['References'].to_s     ).to eq incoming_email.message_id
          expect( held_owner_notice.header['Auto-Submitted'].to_s ).to eq 'auto-replied'
  ...
  let(:expected_sent_email_to)                 { ['raceteam@localhost'] }
  let(:expected_sent_email_cc)                 { '' }
  let(:expected_sent_email_subject)            { "#{expected_conversation.subject_tag} OMG guys I love threadable!" }
  let(:expected_sent_email_smtp_envelope_from) { 'raceteam@localhost' }
  let(:expected_sent_email_reply_to)           { 'UCSD Electric Racing <raceteam@localhost>' }
  ...
  context "when the recipient email address matches an organization" do
    ...
    context 'a parent message cannot be found' do
      ...
      context "and the sender is a organization member" do
        ...
        let(:expected_email_recipients) { ["yan@ucsd.example.com", "tom@ucsd.example.com", "bethany@ucsd.example.com", "bob@ucsd.example.com", "nadya@ucsd.example.com", "lilith@sfhealth.example.com"] }
      ...
      context "and the sender is not a user or a organization member" do
        ...
        let(:expected_conversation){ nil }
        let(:expected_creator     ){ nil }
        let(:expect_message_held_notices_sent){ true }
    ...
    context 'a parent message can be found' do
      ...
      context 'but the creator exists but is not a organization member' do
        ...
        let(:expected_email_recipients){ ["alice@ucsd.example.com", "yan@ucsd.example.com", "tom@ucsd.example.com", "bob@ucsd.example.com", "nadya@ucsd.example.com", "lilith@sfhealth.example.com"] }
      ...
      context 'and the creator is a organization member' do
        ...
        context 'and the body contains commands' do
          ...
          let(:expect_task_to_be_done)           { false }
          let(:expected_sent_email_to)           { ['raceteam+task@localhost'] }
    ...
    context 'the recipient email address contains a group that does not exist and a group that does' do
      let(:recipient) { 'raceteam+hotdogs+fundraising@localhost' }
    ...
    context 'the recipient email address contains only valid groups' do
      ...
      context "and multiple recipients are specified on a single message" do
        let(:recipient)    { 'raceteam+electronics@localhost, raceteam+fundraising@localhost' }
      ...
      context 'and the message is a reply to a conversation that is in different group(s)' do
        ...
        context do
          ...
          let(:expected_groups)              { ['Electronics', 'Fundraising'] }
          let(:expected_sent_email_list_id)  { "UCSD Electric Racing <raceteam.localhost>" }
      ...
      context 'the message is a duplicate of an already received message, but the recipient is to different groups' do
        ...
        before do
          ...
          first_incoming_email = threadable.incoming_emails.create!(params.merge("recipient" => 'raceteam+fundraising@localhost')).first
  ...
  context "and the subject contains [task]" do
    ...
    context "but the message was cc'd to the regular organization address" do
      ...
      let(:cc){ 'UCSD Electric Racing <raceteam@localhost>' }
  ...
  context 'when the from address matches a non-member and the envelope from matches a non-member and the sender matches a member' do
    ...
    let(:expected_email_recipients){ ["alice@ucsd.example.com", "yan@ucsd.example.com", "tom@ucsd.example.com", "bethany@ucsd.example.com", "nadya@ucsd.example.com", "lilith@sfhealth.example.com"] }
  ...
  context 'when the from address matches a member and the envelope from matches a non-member but the sender matches a non-member' do
    ...
    let(:expected_email_recipients){ ["alice@ucsd.example.com", "yan@ucsd.example.com", "tom@ucsd.example.com", "bethany@ucsd.example.com", "bob@ucsd.example.com", "nadya@ucsd.example.com", "lilith@sfhealth.example.com"] }
  ...
  context 'when there are non-members in the to header, and the organization is in the CC' do
    ...
    let(:expected_sent_email_cc){ 'UCSD Electric Racing <raceteam@localhost>' }
  ...
  context 'header rearranging: with groups' do
    ...
    context 'when there are only members in the to header, and the group and organization are in the CC' do
      let(:to) { "Tom Canver <tom@ucsd.example.com>" }
      let(:cc) { '"UCSD Electric Racing: Electronics" <raceteam+electronics@localhost>, UCSD Electric Racing <raceteam@localhost>' }
  ...
  context 'when the message is a reply to a non-task conversation but the message was sent to the organization task email addresss' do
    ...
    let(:to)         { 'UCSD Electric Racing Tasks <raceteam+task@localhost>' }
    let(:in_reply_to){ expected_parent_message.message_id_header }
    let(:references) { '' }
  ...
  context 'when the message is a reply to a task but the message was sent to the organization non-task email addresss' do
    let(:recipient)  { 'raceteam@localhost' }
    let(:to)         { 'UCSD Electric Racing Tasks <raceteam@localhost>' }
    let(:in_reply_to){ expected_parent_message.message_id_header }
    let(:references) { '' }
  ...
  context 'when the recipient is at covered.io' do
    ...
    let(:expected_sent_email_to)   { ["raceteam@localhost", "raceteam@example.com"] }
    it 'replaces covered.io with threadable.com' do
      validate! :delivered
    end
  end
  ...
  context 'when the organization holds all messages' do
    ...
    context 'and the sender is an owner' do
      ...
      let(:expected_email_recipients) { ["yan@ucsd.example.com", "tom@ucsd.example.com", "bethany@ucsd.example.com", "nadya@ucsd.example.com", "bob@ucsd.example.com", "lilith@sfhealth.example.com"] }
