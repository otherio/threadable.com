  ...
  when_signed_in_as 'alice@ucsd.example.com' do
    ...
    describe 'conversation_message' do
      ...
      def expect_email!
        ...
        expect( email.header['Reply-To'].to_s    ).to eq 'UCSD Electric Racing Tasks <raceteam+task@localhost>'
        expect( email.header['To'].to_s          ).to eq 'UCSD Electric Racing Tasks <raceteam+task@localhost>'
        ...
        expect( email.header['List-ID'].to_s      ).to eq 'UCSD Electric Racing <raceteam.localhost>'
        ...
        expect( email.header["List-Post"].to_s        ).to eq "<mailto:raceteam@localhost>, <#{compose_conversation_url(organization, 'my')}>"
      end
      ...
      context "when there are multiple people in the TO and CC headers" do
        ...
        it "should filter out organization members from the TO and CC headers" do
          ...
          expect( email.header['To'].to_s ).to     include 'UCSD Electric Racing <raceteam@localhost>'
      ...
      context 'when the organization is in the CC header and there are multiple people in the TO and CC headers' do
        ...
        it "should include the organization in the CC header" do
          ...
          expect( email.header['To'].to_s ).to_not be_blank
          expect( email.header['Cc'].to_s ).to include 'UCSD Electric Racing <raceteam@localhost>'
        end
      end
      context 'when the organization is in the BCC header and there are multiple people in the TO and CC headers' do
        ...
        it "should not include the organization email address in the TO or CC headers" do
          ...
          expect( email.header['To'].to_s ).to_not be_blank
          expect( email.header['Cc'].to_s ).to_not include 'UCSD Electric Racing <raceteam@localhost>'
          expect( email.header['To'].to_s ).to_not include 'UCSD Electric Racing <raceteam@localhost>'
        end
      end
    end
