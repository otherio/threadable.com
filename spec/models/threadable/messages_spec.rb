...
describe Threadable::Messages, :type => :model do
  ...
  describe 'all' do
    it 'returns all messages' do
      ...
      expect(Threadable::Message).to receive(:new).with(threadable, 1).and_return(1)
      expect(Threadable::Message).to receive(:new).with(threadable, 2).and_return(2)
      expect(Threadable::Message).to receive(:new).with(threadable, 3).and_return(3)
  ...
  describe 'latest' do
    it 'returns the latest message' do
      ...
      expect(Threadable::Message).to receive(:new).with(threadable, message_record).and_return(message)
      expect(messages.latest).to eq message

      expect(scope).to receive(:last).and_return(nil)
      expect(Threadable::Message).to_not receive(:new)
    ...
    context 'when the message is not found' do
      ...
      it 'raises a Threadable::RecordNotFound error' do
        expect{ messages.find_by_id!(66) }.to raise_error(
          Threadable::RecordNotFound, 'unable to find conversation message with id: 66')
      end
    end
  end
    ...
    context 'when the message is not found' do
      ...
      it 'raises a Threadable::RecordNotFound error' do
        expect{ messages.find_by_message_id_header!('<xxxx@x.com>') }.to raise_error(
          Threadable::RecordNotFound, 'unable to find conversation message message id header: <xxxx@x.com>')
      end
    end
  end
    ...
    context 'when the message is not saved' do
      ...
      it 'return the message' do
        expect{ messages.create!(my_new: 'message') }.to raise_error(
          Threadable::RecordInvalid, "Message invalid: cannot be lame and cannot be stupid")
      end
    end
  end
