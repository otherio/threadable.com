...
describe ProcessIncomingIntegrationHookWorker do
  ...
  let(:incoming_integration_hook){ double(:incoming_integration_hook, provider: 'trello', external_conversation_id: 'EXTERNAL_CONVERSATION_ID') }
  ...
  context "when another message with the same message_id is not being processed" do
    before do
      expect(Threadable.redis).to receive(:setnx).with('ProcessIncomingIntegrationHookWorker:trello-EXTERNAL_CONVERSATION_ID', Time.now.to_i).and_return(true)
    end

    it "should find the incoming integration_hook record and pass it to threadable.process_incoming_integration_hook" do
      expect(Threadable.redis).to receive(:expire).with('ProcessIncomingIntegrationHookWorker:trello-EXTERNAL_CONVERSATION_ID', 30).and_return(true)
      expect(incoming_integration_hook).to receive(:process!)
      expect(Threadable.redis).to receive(:del).with('ProcessIncomingIntegrationHookWorker:trello-EXTERNAL_CONVERSATION_ID')
      perform!(42)
  ...
  context "when another message with the same message_id is being processed" do
    before do
      expect(Threadable.redis).to receive(:setnx).with('ProcessIncomingIntegrationHookWorker:trello-EXTERNAL_CONVERSATION_ID', Time.now.to_i).and_return(false)
    end
