...
describe ProcessIncomingEmailWorker do
  ...
  context "when another message with the same message_id is not being processed" do
    before do
      expect(Threadable.redis).to receive(:setnx).with('ProcessIncomingEmailWorker:<9fe7229c2e5c88cbde6b5a6a4c99bc3fb7c2144e@example.com>', Time.now.to_i).and_return(true)
    end

    it "should find the incoming email record and pass it to threadable.process_incoming_email" do
      expect(Threadable.redis).to receive(:expire).with('ProcessIncomingEmailWorker:<9fe7229c2e5c88cbde6b5a6a4c99bc3fb7c2144e@example.com>', 30).and_return(true)
  ...
  context "when another message with the same message_id is being processed" do
    before do
      expect(Threadable.redis).to receive(:setnx).with('ProcessIncomingEmailWorker:<9fe7229c2e5c88cbde6b5a6a4c99bc3fb7c2144e@example.com>', Time.now.to_i).and_return(false)
    end
