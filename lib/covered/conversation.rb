class Covered::Conversation < ActiveRecord::Base

  belongs_to :project, :class_name => 'Covered::Project'
  belongs_to :creator, :class_name => 'Covered::User'
  has_many :messages, -> { order created_at: :asc }
  has_many :events, class_name: 'Covered::Conversation::Event'
  has_many :participants, ->{ uniq }, through: :messages, source: :user
  # TODO recipients will eventually have a scope that removes members who have muted this conversation
  has_many :recipients, through: :project, class_name: 'Covered::User', source: 'members_who_get_email'

  default_scope -> { order('conversations.updated_at DESC') }
  scope :with_slug, ->(slug){ where(slug: slug).limit(1) }
  scope :task, ->{ where(type: 'Covered::Task') }
