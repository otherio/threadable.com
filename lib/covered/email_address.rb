class Covered::EmailAddress < ActiveRecord::Base
  ...
  scope :for_user, ->(user){
    where(user_id: user.is_a?(Covered::User) ? user.id : user)
  }
  ...
  def ensure_only_one_primary
    if self.class.primary.for_user(user_id).address_not(self.address).present?
